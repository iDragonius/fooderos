...
import op from 'prettier/esm/parser-markdown.mjs'
const customStyles = {
    valueContainer: () => ({
        height: '54px',
        display: 'flex',
...
const Variant = ({ variantKey, setDel, types, i }) => {
    ...
    useEffect(() => {
        setName({
            label: Object.keys(variants)[i],
            value: Object.keys(variants)[i],
        })
        const temp = []
        let optionId = 0
        if (options[Object.keys(variants)[i]]) {
            Object.keys(options[Object.keys(variants)[i]]).map((opt) => {
                temp.push(
                    <Option
                        type={Object.keys(variants)[i]}
                        value={option}
                        keyData={opt}
                        setDeleted={setDeleteId}
                        del={deleteId}
